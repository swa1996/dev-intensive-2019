...
class ChatItemTouchHelperCallBack(val adapter: ChatAdapter, val swipeListener: (ChatItem) -> Unit) :
    ...
    private fun drawIcon(c: Canvas, itemView: View, dX: Float) {
        ...
        val spacing = itemView.resources.getDimensionPixelSize(R.dimen.spacing_normal_16)
        val margin = (itemView.bottom - itemView.top - iconSize) / 2

        with(iconBounds) {
            left = itemView.right + dX.toInt() + spacing
            top = itemView.top + margin
            right = itemView.right + dX.toInt() + iconSize + spacing
